...
/*   Updated: 2023/04/25 15:43:25 by nimai            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* void	g_positive_low_b(t_f *f, t_nmbs nbr1, t_nmbs nbr2)
{
	...
	if (nbr1.grd[0] > nbr1.grd[nbr1.len] && nbr1.colour < nbr2.colour && f->flag == 0)
	{
		while (nbr1.len >= 0)
		{
			f->n[f->pwd_j][f->pwd_i].grd[c] = nbr1.grd[nbr1.len];
			nbr1.len--;
			c++;
		}
	}
	else if (nbr1.grd[0] > nbr1.grd[nbr1.len] && nbr1.colour < nbr2.colour && f->flag == 0)//koremo kann
	{
		while (nbr1.len >= 0)
		{
			f->n[f->pwd_j][f->pwd_i].grd[c] = nbr1.grd[nbr1.len];
